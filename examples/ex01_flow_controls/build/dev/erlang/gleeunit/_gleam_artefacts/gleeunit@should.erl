...
-spec be_ok({ok, DSD} | {error, any()}) -> DSD.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DSI}) -> DSI.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DSL)) -> DSL.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
